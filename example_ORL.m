...
load('extendedORL.mat', 'dirName');
userName = dirName;
classNum = numel(userName);

% output path
destDir = cell(classNum, 1);
destRoot = fullfile(currentPath, 'result', 'ORL') ;
for i = 1 : classNum
...
%% get image groups
[ ImageMat ] = get_images_ORL(imagePath, para);
...
%% record test results
outputFileName = fullfile(destRoot, 'ORL_parameters.txt'); 
