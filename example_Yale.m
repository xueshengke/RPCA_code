...
load('croppedYaleB.mat', 'className');
userName = className;
classNum = min(numel(userName), 15); % only select first 15 categories of YaleB
...
%% get image groups
[ ImageMat ] = get_images_Yale(imagePath, para);
...
%% record test results
outputFileName = fullfile(destRoot, 'Yale_parameters.txt'); 
